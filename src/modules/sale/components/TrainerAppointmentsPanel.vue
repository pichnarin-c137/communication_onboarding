<template>
  <div class="space-y-4">
    ...
    <!-- Loading -->
    <div v-if="loading && appointments.length === 0" class="bg-white border border-gray-200 rounded-xl p-5">
      <div v-for="i in 4" :key="i" class="flex items-center gap-3 py-3 border-b border-gray-100 last:border-b-0 animate-pulse">
        ...
        <div class="w-16 h-5 bg-gray-100 rounded-full" />
      </div>
    </div>

    <!-- Empty -->
    <div v-else-if="!loading && appointments.length === 0" class="bg-white border border-gray-200 rounded-xl p-10 text-center">
      <CalendarDaysIcon class="w-9 h-9 text-gray-300 mx-auto mb-2" />
      <p class="text-sm text-gray-500">No appointments {{ statusFilter ? `with this status` : '' }} yet.</p>
    </div>
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-gray-900 truncate">
            {{ appt.title || appt.client?.company_name || 'Appointment' }}
          </p>
          <p class="text-xs text-gray-500 mt-0.5">
            {{ formatDate(appt.scheduled_date) }}
            <span class="text-gray-300 mx-1">·</span>
            {{ formatTime(appt.scheduled_start_time) }}
            <span v-if="appt.client?.company_name" class="text-gray-400 ml-2">{{ appt.client.company_name }}</span>
          </p>
        </div>
...
async function loadPage(page = 1) {
  ...
  try {
    ...
    const res = await saleService.getAppointments(params)
