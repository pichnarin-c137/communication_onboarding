<template>
  <div class="grid grid-cols-3 gap-3">
    ...
    <!-- Sessions -->
    <button
      class="flex items-center gap-3 bg-white border border-slate-200 rounded-lg px-4 py-3 text-left hover:border-slate-300 hover:bg-slate-50/70 transition-colors group"
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs text-slate-500 mb-0.5">Sessions</p>
        <div v-if="loading" class="h-3.5 w-16 bg-gray-100 rounded animate-pulse" />
        <p v-else class="text-sm font-semibold text-slate-900 truncate">
          {{ sessionsCount > 0 ? `${sessionsCount} linked` : 'None yet' }}
        </p>
      </div>
      <svg class="w-4 h-4 text-slate-300 group-hover:text-slate-500 shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      <div class="w-8 h-8 rounded-md flex items-center justify-center shrink-0" :class="feedbackIconBg">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs text-slate-500 mb-0.5">Feedback</p>
        <div v-if="loading" class="h-3.5 w-20 bg-gray-100 rounded animate-pulse" />
        <div v-else class="flex items-center gap-1.5">
          <span
            v-if="feedbackStatus !== 'not_requested'"
            class="w-1.5 h-1.5 rounded-full shrink-0"
            :class="feedbackStatus === 'submitted' ? 'bg-emerald-500' : 'bg-amber-500'"
          />
          <p class="text-sm font-semibold text-slate-900 truncate">{{ feedbackLabel }}</p>
        </div>
      </div>
      <svg class="w-4 h-4 text-slate-300 group-hover:text-slate-500 shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
    ...
    >
      <div class="w-8 h-8 rounded-md flex items-center justify-center shrink-0" :class="revisionsIconBg">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs text-slate-500 mb-0.5">Revisions</p>
        <div v-if="loading" class="h-3.5 w-16 bg-gray-100 rounded animate-pulse" />
        <div v-else class="flex items-center gap-1.5">
          <span
...
const feedbackIconBg = computed(() => {
  if (feedbackStatus.value === 'submitted') return 'bg-slate-100'
  if (feedbackStatus.value === 'sent') return 'bg-slate-100'
  return 'bg-slate-100'
})

const feedbackIconColor = computed(() => {
  if (feedbackStatus.value === 'submitted') return 'text-slate-600'
  if (feedbackStatus.value === 'sent') return 'text-slate-600'
  return 'text-slate-400'
})

const revisionsIconBg = computed(() =>
  pendingRevisions.value > 0 ? 'bg-slate-100' : 'bg-slate-100'
)

const revisionsIconColor = computed(() =>
  pendingRevisions.value > 0 ? 'text-slate-600' : 'text-slate-400'
)
